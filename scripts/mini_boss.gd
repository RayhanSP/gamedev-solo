...
# ==========================================
# --- PENERIMA DAMAGE & EFEK DARI ITEM ---
# ==========================================

# 1. Busi & Metal Gear masuk ke sini (Standard Damage)
...
# 2. Ban akan manggil fungsi ini
func apply_knockback(strength: float):
	if state == "DEAD": return
	
	# Bos gak mempan didorong mundur, jadi kita ganti efeknya jadi STUN
	if state == "WIND_UP" or state == "CHARGE":
		apply_stun(5.0) # Counter Sukses!
	else:
		apply_stun(2.5) # Kena ban saat jalan biasa
		
	hp -= 5 # Ekstra damage dari ban sesuai request lu
	flash_white()
	if hp <= 0: die()

# 3. Aki / Battery akan manggil fungsi ini
...
func apply_battery_effect():
	if is_slowed: return # Biar efeknya gak ketumpuk-tumpuk
	is_slowed = true
